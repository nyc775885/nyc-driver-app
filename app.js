...
console.log("%cNYC Driver Tracker — version v83","color:#00D4FF;font-weight:bold;font-size:14px");
          ...
          React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 262}}
            ...
            , dashV==="month" ? (
              React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 265}}
                ...
                , (function(){
                  var isEv=veh.type==="electric";var fuelCat=isEv?"charging":"fuel";var unit=isEv?"/kWh":"/Gal";var unitName=isEv?"kWh":"Gal";
                  // Get last 30 days of fuel/charging entries with qty
                  var nowD=new Date();var cutoff=new Date(nowD.getFullYear(),nowD.getMonth(),nowD.getDate()-30);
                  var fi=el.filter(function(e){if(e.category!==fuelCat||!e.qty||+e.qty<=0||!e.date)return false;var ed=new Date(e.date.split("-")[0],+e.date.split("-")[1]-1,+e.date.split("-")[2]);return ed>=cutoff;}).sort(function(a,b){return a.date.localeCompare(b.date);});
                  ...
                  return React.createElement(Card, { style: {marginBottom:8,padding:"12px 14px",cursor:"pointer"}, onClick: function(){setTrendOpen(!trendOpen);}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 281}}
                    , React.createElement('div', { style: {display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 281}}
                      , React.createElement('div', { style: {fontSize:13,fontWeight:700,color:"#FFD700"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 281}}, "⛽ " , lang==="en"?(isEv?"Charging Price · 30d":"Gas Price · 30d"):(isEv?"充电价格 · 近30天":"油价 · 近30天"))
                    ...
                    , trendOpen ? React.createElement('div', { style: {marginTop:12,paddingTop:10,borderTop:"1px solid "+C.border}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 281}}
                      , React.createElement('div', { style: {fontSize:11,color:C.text3,marginBottom:6}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 281}}, lang==="en"?"All 30-day fills":"全部 30 天充电记录")
        ...
        React.createElement('div', { style: {position:"fixed",inset:0,zIndex:400,display:"flex"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 596}}
          , React.createElement('div', { style: {width:"60%",maxWidth:220,background:C.bg2,height:"100%",overflowY:"auto",borderRight:"1px solid "+C.border,display:"flex",flexDirection:"column",paddingBottom:"70px"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 597}}
            , React.createElement('div', { style: {padding:"20px 18px 16px",borderBottom:"1px solid "+C.border}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 598}}
              ...
              , React.createElement('div', { style: {fontSize:11,color:C.text3,marginTop:2}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 600}}, "NYC RIDESHARE TRACKER · v1.3.3"    )
            )
        ...
        React.createElement('div', { style: {position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,paddingBottom:"80px"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 784}}
          , React.createElement('div', { style: {background:C.bg2,borderRadius:16,width:"100%",maxWidth:600,border:"1px solid "+C.border,maxHeight:"calc(90vh - 80px)",display:"flex",flexDirection:"column",overflow:"hidden"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 785}}
            ...
            , React.createElement('button', { onClick: function(){
                try{
                  var slKey=function(x){return (x.month||"")+"|"+(x.platform||"")+"|"+((+x.grossFare||0).toFixed(2));};
                  var elKey=function(x){return (x.date||"")+"|"+(x.category||"")+"|"+((+x.amount||0).toFixed(2))+"|"+(x.notes||"")+"|"+(x.statementMonth||"")+"|"+((+x.qty||0).toFixed(2));};
                  var wlKey=function(x){return (x.weekStart||"")+"|"+(x.platform||"")+"|"+((+x.miles||0).toFixed(0))+"|"+((+x.trips||0).toFixed(0))+"|"+((+x.hours||0).toFixed(1));};
                  var dedup=function(arr,keyFn){var seen={},out=[],dups=0;arr.forEach(function(item){var k=keyFn(item);if(seen[k]){dups++;}else{seen[k]=true;out.push(item);}});return {out:out,dups:dups};};
                  var slR=dedup(sl,slKey),elR=dedup(el,elKey),wlR=dedup(wl,wlKey);
                  var total=slR.dups+elR.dups+wlR.dups;
                  if(total===0){alert(lang==="en"?"✓ No duplicates found":"✓ 没有发现重复数据");return;}
                  var msg=lang==="en"
                    ?("Found duplicates:\n• "+slR.dups+" monthly statements\n• "+elR.dups+" expenses\n• "+wlR.dups+" weekly logs\n\nRemove them?")
                    :("发现重复数据：\n• 月度账单 "+slR.dups+" 条\n• 支出 "+elR.dups+" 条\n• 周记录 "+wlR.dups+" 条\n\n是否删除？");
                  if(!confirm(msg))return;
                  setSl(slR.out);setEl(elR.out);setWl(wlR.out);
                  alert(lang==="en"?"✓ Removed "+total+" duplicate entries":"✓ 已清除 "+total+" 条重复数据");
              ...
              , React.createElement('div', { style: {fontSize:15,fontWeight:700,color:"#FFB300",marginBottom:3}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 802}}, lang==="en"?"🧹 Deduplicate Records":"🧹 清理重复数据")
              , React.createElement('div', { style: {fontSize:12,color:C.text3}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 803}}, lang==="en"?"Find and remove duplicate statements/expenses/weekly logs":"查找并删除重复的月账单/支出/周记录")
            )
