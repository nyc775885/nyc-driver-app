...
console.log("%cNYC Driver Tracker — version v199","color:#00D4FF;font-weight:bold;font-size:14px");
...
function App() {
  ...
  function loadFromDrive(tok){
    ...
    getDriveData(tok,function(fileId,folderId,modTime){
      ...
      .then(function(data){
        ...
        if(typeof data.stdDed==="number")setStdDed(data.stdDed);if(typeof data.mtaRate==="number")setMtaRate(data.mtaRate);if(Array.isArray(data.dl))setDl(data.dl);if(data.driverType==="rideshare"||data.driverType==="taxi")setDriverType(data.driverType);
  ...
  },[wl,sl,el,fl,ll,veh,cc,custGroups,reminders,custPlat,custBrands,custLicTypes,custLoanTypes,favNotes,notes,incGoal,seRate,fedRate,stateRate,stdDed,mtaRate,dl,driverType]);
    ...
    getDriveData(accessToken,function(fileId,folderId,driveModTime){
      ...
      .then(function(cloudData){
        ...
        if(cloudModTime > localModTime){
          ...
          if(typeof cloudData.stdDed==="number")setStdDed(cloudData.stdDed);if(typeof cloudData.mtaRate==="number")setMtaRate(cloudData.mtaRate);
      ...
      , showDrawer ? (
        React.createElement('div', { style: {position:"fixed",inset:0,zIndex:400,display:"flex"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 596}}
          , React.createElement('div', { style: {width:"60%",maxWidth:220,background:C.bg2,height:"100%",overflowY:"auto",borderRight:"1px solid "+C.border,display:"flex",flexDirection:"column",paddingBottom:"70px"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 597}}
            ...
            , React.createElement('div', { style: {fontSize:10,color:C.text3,textAlign:"center",padding:"12px 18px 16px",borderTop:"1px solid "+C.border,letterSpacing:0.5} }, "NYC RIDESHARE TRACKER · v2.6.7"    )
          )
          ...
          , React.createElement('div', { style: {background:C.bg2,borderRadius:16,width:"100%",maxWidth:600,border:"1px solid "+C.border,maxHeight:"calc(85vh - 80px)",display:"flex",flexDirection:"column",overflow:"hidden"}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 768}}
            ...
            , React.createElement('div', { style: {background:C.bg3,border:"1px solid "+C.border,borderRadius:12,padding:14,marginBottom:10} }
              ...
              , React.createElement('button', { onClick: function(){setShowPlatMgr(false);setPasteUberTaxText("");setPasteUberTaxResult(null);setShowPasteUberTax(true);}, style: {width:"100%",background:"#1A1000",border:"1px solid #3A2800",borderRadius:10,padding:"10px 14px",color:"#FFB300",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"} }
                , React.createElement('span', null, "📊 " , lang==="en"?"Annual Tax Summary":"年度税务总结")
                , React.createElement('span', {style:{color:"#9A7A40",fontSize:14}}, "→")
              )
            )
