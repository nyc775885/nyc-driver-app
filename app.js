...
function App() {
  ...
  var mDailies=useMemo(function(){return dl.filter(function(d){return d.date&&d.date.slice(0,7)===mo;});},[dl,mo]);var mDailyInc=mDailies.reduce(function(s,d){return s+(+d.cash||0)+(+d.card||0)+(+d.tips||0);},0);var mDailyLease=mDailies.reduce(function(s,d){return s+(+d.lease||0);},0);var tInc=mStmts.reduce(function(s,x){return s+(+x.grossFare||0)+(+x.tips||0)+(+x.bonus||0)+(+x.tollReimbursed||0)+(+x.otherIncome||0);},0)+mDailyInc;
          ...
          React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 342}}
            ...
            , (function(){
                ...
                return React.createElement('div',{style:{marginBottom:10}},
                  ...
                  sortedCats.length>0?React.createElement('div',{style:{display:"flex",gap:6,overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:2}},
                    hasFilter?React.createElement('button',{onClick:function(){setExpFilterCat([]);},style:{flexShrink:0,background:"#2A1010",border:"1px solid #5A2020",color:"#FF7060",fontSize:11,padding:"4px 10px",borderRadius:14,cursor:"pointer",whiteSpace:"nowrap"}},lang==="en"?"× Clear":"× 清除"):null,
                    sortedCats.map(function(k){
                      ...
                      return React.createElement('button',{
                        ...
                        style:{flexShrink:0,background:sel?"#0A2040":C.bg3,border:"1px solid "+(sel?"#00D4FF":C.border),color:sel?"#00D4FF":C.text2,fontSize:11,padding:"4px 10px",borderRadius:14,cursor:"pointer",whiteSpace:"nowrap",fontWeight:sel?700:400}
                      },icon," ",label," ",React.createElement('span',{style:{opacity:0.6,fontSize:10}},catCount[k]));
                    })
                  ):null
                );
              }())
              ...
              return React.createElement('div', {style:{background:"#0A1828",border:"1px solid #2A4A6A",borderRadius:10,padding:"8px 10px",marginBottom:6}}
                ...
                , React.createElement('div', {style:{display:"flex",gap:6,flexWrap:"wrap"}}
                  , distinctRecent.map(function(m,idx){
                      ...
                      }, isCurrent?"✓ ":"", "$" + valStr, isUnitMode?React.createElement('span',{style:{fontSize:10,color:C.text3,marginLeft:1}},unitSuffix):null, React.createElement('span', {style:{fontSize:9,color:C.text3,marginLeft:4}}, m.date||""));
                    })
                )
              );
            }())
